        ...
        if (event.message.type === "text") {
          ...
          if (event.source.type === "group") {
            ...
            if (validateEventType.includes(event.message.type)) {
              const resGetContent = await line.getContent(
                event.message,
                event.message.id
              );
              ...
              const publicUrl = await firebase.saveImageToStorage(
                event.source.groupId,
                resGetContent.fileName,
                resGetContent.binary
              );
              await firebase.insertImageGroup(
                event.source.groupId,
                event.message.id,
                publicUrl
              );
