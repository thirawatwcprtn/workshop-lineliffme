  ...
  try {
    ...
    if (profile == undefined) {
      ...
      if (response.status === 200) {
        ...
        throw new Error(
          `Failed to fetch user profile. API responded with status: ${response.status}`
        );
      }
    }
    return profile;
...
exports.isAnimationLoading = async (userId) => {
  try {
    ...
    if (response.status === 202) {
      ...
      throw new Error(
        `Failed to send reply. API responded with status: ${response.status}`
      );
    }
  } catch (error) {
...
async function issueStatelessAccessToken() {
  try {
    ...
    if (
      ...
      console.log(
        `[issueStatelessAccessToken] : ${response.data.access_token} `
      );
      return response.data.access_token;
    } else {
      throw new Error(
        "Failed to obtain access token, check response for details."
      );
    }
  } catch (error) {
